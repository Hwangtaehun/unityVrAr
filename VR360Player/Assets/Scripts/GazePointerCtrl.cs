...
{
    public Transform uiCanvas;          //일정 시간 동안 시건이 머무는 것을 보여주기 위한 UI
    public Image gazeImg;               //시선이 머무는 동안 변화를 표현하기 위한 UI image 컴포넌트

    Vector3 defaultScale;               //UI 기본 스케일을 저장해두기 위한 값
    public float uiScaleVal = 1.0f;     //UI 카메라가 1m일 때 배율 
    ...
    public float gazeChargeTime = 3.0f; //시선이 머문 시간을 체크하기 위한 기준 시간 3초(필요에 따라 수정)
    public Video360Play vp360;         //360 스피어에 추가된 영상 플레이 기능

    void Start()
    {
        defaultScale = uiCanvas.localScale; //오브젝트가 갖는 기본 스케일 값
        curGazeTime = 0;                    //시선 유지하는지 체크하기 위한 변수 초기화
    }
    ...
    void Update()
    {
        ...
        else //4.아무것도 부딪히지 않으면 기본 스케일 값으로 uiCanva의 크기를 조절한다.
        {
        ...
        gazeImg.fillAmount = curGazeTime / gazeChargeTime;         //ui Image의 fillAmount를 업데이트한다.
    ...
    void HitObjChecker(GameObject hitObj, bool isActive)
    {
        ...
        // 정해진 시간이 되면 360 스피어에 특정 클립 전호를 전달해 플레이한다.
